...
But there is an even simpler way. What if we could just output the current file? It would contain all the source code including the flag. We can use the `__FILE__` magic constant to get the current file path, and `file_get_contents()` which is somehow not on the diasbled functions list. 

```php
?><?=file_get_contents(__FILE__);
```

This will print the entire source code of the PHP script, including the flag. And it is very consise, way below our 125 byte limit, which can help us, as it gives us more room for potential encoding.
...
We have sucessfully created a banned character with just symbols. Now we can use this to create a string with all the letters of the alphabet. This would be tiresome to do manually, so I wrote a small script to generate the combinations for me.
...
However, we run into an issue. Converting `__FILE__` into our symbol payload, we see that PHP no longer recognizes it as a magic constant. Only functions seem to be recognized this way. We need to find an alternative to `__FILE__`. Looking at the PHP documentation, the functon `get_included_files()` seems promissing. This function returns an array of all included files. The first element being our current file.
